...
import { VerticalFraction, toFraction } from '../utils/fractionUtils';
...
export default function DeterminationRulesMode({ sidePanelOpen, setSidePanelOpen, onParamChange }) {
  ...
  const slopeCFrac = toFraction(rawSlopeC);
  ...
  useEffect(() => {
    ...
    if (subTab === 'twoPoints') {
      onParamChange(rawSlopeA, interceptA, [
        { x: p1.x, y: p1.y, label: 'P1' },
        { x: p2.x, y: p2.y, label: 'P2' }
      ], isRevealed);
    } else if (subTab === 'slopeAndPoint') {
      onParamChange(fixedA, interceptB, [{ x: singleP.x, y: singleP.y, label: 'P' }], isRevealed);
    } else if (subTab === 'intercepts') {
      onParamChange(rawSlopeC, yIntercept, [
        { x: xIntercept, y: 0, label: 'x절편' },
        { x: 0, y: yIntercept, label: 'y절편' }
      ], isRevealed);
    }
  }, [subTab, p1, p2, fixedA, singleP, xIntercept, yIntercept, rawSlopeA, interceptA, interceptB, rawSlopeC, isRevealed, onParamChange]);
      ...
      <div className="side-panel-content">
        ...
        <div className="story-selector">
          <button
            ...
            onClick={() => handleSubTabChange('twoPoints')}
          >
            <span>1. 두 점 주어질 때</span>
          </button>
          <button
            className={`story-tab ${subTab === 'slopeAndPoint' ? 'active' : ''}`}
            onClick={() => handleSubTabChange('slopeAndPoint')}
          >
            <span>2. 기울기+1점</span>
          </button>
          <button
            className={`story-tab ${subTab === 'intercepts' ? 'active' : ''}`}
            onClick={() => handleSubTabChange('intercepts')}
          >
            <span>3. x절편+y절편</span>
          </button>
        </div>
        ...
        {subTab === 'twoPoints' && (
          <div className="concept-card" style={{ background: '#f0fdf4', borderColor: '#bbf7d0' }}>
              ...
              <div style={{ background: '#ffffff', padding: '0.75rem', borderRadius: '8px', border: '1px solid #dcfce7' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <input
                    ...
                    value={p1.x}
                    onChange={(e) => {
                      setP1({ ...p1, x: Number(e.target.value) });
                      setIsRevealed(false);
                    }}
                  />
                  ...
                  <input
                    ...
                    value={p1.y}
                    onChange={(e) => {
                      setP1({ ...p1, y: Number(e.target.value) });
                      setIsRevealed(false);
                    }}
                  />
                </div>
              ...
              <div style={{ background: '#ffffff', padding: '0.75rem', borderRadius: '8px', border: '1px solid #dcfce7' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <input
                    ...
                    value={p2.x}
                    onChange={(e) => {
                      setP2({ ...p2, x: Number(e.target.value) });
                      setIsRevealed(false);
                    }}
                  />
                  ...
                  <input
                    ...
                    value={p2.y}
                    onChange={(e) => {
                      setP2({ ...p2, y: Number(e.target.value) });
                      setIsRevealed(false);
                    }}
                  />
                </div>
            ...
            >
              {isRevealed ? '🔄 위치 재설정' : '✨ 두 점을 지나는 직선 그리기 & 식 완성!'}
            </button>

            {!isRevealed ? (
              <div className="reveal-placeholder-card">
                💡 점 P1과 P2의 좌표를 설정한 후<br />
                <strong>[직선 그리기 & 식 완성]</strong> 버튼을 누르면 그래프와 일차함수 식이 나타납니다!
              </div>
            ) : (
        ...
        {subTab === 'slopeAndPoint' && (
          <div className="concept-card" style={{ background: '#eff6ff', borderColor: '#bfdbfe' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '0.5rem' }}>
              <div className="slider-box">
                ...
                <input
                  ...
                  value={fixedA}
                  onChange={(e) => {
                    setFixedA(Number(e.target.value));
                    setIsRevealed(false);
                  }}
                />
              </div>
              ...
              <div style={{ background: '#ffffff', padding: '0.75rem', borderRadius: '8px', border: '1px solid #dbeafe' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <input
                    ...
                    value={singleP.x}
                    onChange={(e) => {
                      setSingleP({ ...singleP, x: Number(e.target.value) });
                      setIsRevealed(false);
                    }}
                  />
                  ...
                  <input
                    ...
                    value={singleP.y}
                    onChange={(e) => {
                      setSingleP({ ...singleP, y: Number(e.target.value) });
                      setIsRevealed(false);
                    }}
                  />
                </div>
            ...
            <button
              ...
              onClick={() => setIsRevealed(!isRevealed)}
            >
              {isRevealed ? '🔄 다시 설정하기' : '✨ 기울기와 점을 지나는 직선 그리기 & 식 완성!'}
            </button>

            {!isRevealed ? (
              <div className="reveal-placeholder-card">
                💡 기울기와 점 P의 위치를 조절한 후<br />
                <strong>[직선 그리기 & 식 완성]</strong> 버튼을 누르면 그래프와 일차함수 식이 나타납니다!
              </div>
            ) : (
              <div className="reveal-anim-container">
        ...
        {subTab === 'intercepts' && (
          <div className="concept-card" style={{ background: '#faf5ff', borderColor: '#e9d5ff' }}>
            ...
            <p style={{ fontSize: '13px', color: '#7e22ce', lineHeight: '1.4' }}>
              직선이 x축과 만나는 점(x절편) 및 y축과 만나는 점(y절편)만 찾으면 바로 함수식이 완성됩니다!
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '0.5rem' }}>
              ...
              <div className="slider-box">
                ...
                <input
                  ...
                  value={yIntercept}
                  onChange={(e) => {
                    setYIntercept(Number(e.target.value));
                    setIsRevealed(false);
                  }}
                />
              </div>
            ...
            <button
              ...
              onClick={() => setIsRevealed(!isRevealed)}
            >
              {isRevealed ? '🔄 다시 설정하기' : '✨ 두 절편을 지나는 직선 그리기 & 식 완성!'}
            </button>
